...
Aplicando o processamento descrito nas Equações \ref{eq:deslocarH120} \ref{eq:deslocarH240} sobre a imagem original da Figura \ref{fig:figuraSimulador}, é possível observar que as características da imagem são preservadas, apesar de serem apresentadas em outras cores. Para essa imagem, as características também podem ser observadas por daltônicos do tipo protanopia e deuteranopia, conforme é possível verificar aplicando a simulação de daltonismo. A Figura \ref{fig:figuraSimulacaoH} ilustra essas observações.
...
(c)
\endminipage\hfill
\caption{Resultado da correção da imagem (a) com equalização (b) e sem equalização (c). (adaptado de \citeonline{jlee2008})}
