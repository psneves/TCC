...
\begin{figure}[!htb]
\centering \includegraphics[width=0.90\textwidth]{figuraDeslocarHSeV.jpg}
...
\begin{figure}[!htb]
\centering \includegraphics[width=0.90\textwidth]{figuraDeslocarHSeI.jpg}
...
Foram implementados outros dois processamentos com o objetivo de deslocar os valores da matiz, um com o objetivo de deslocar os valores próximos a $60^o$ e o outro com o objetivo de deslocar os valores próximos a região que mais possui valores incidentes em H, ou seja, o pico do histograma. Para esses métodos, os valores próximos foram definidos como os valores que estão até $60^o$ de distância do ângulo (e.g. valores próximos a $60^o$ são os valores que estão no intervalo $[0^o;120^o]$).

O valor que representa o valor de amarelo em H é $60^o$. Amarelo está exatamente entre o verde e o vermelho. A maioria dos daltônicos possuem dificuldade para distinguir verde e vermelho, conforme descrito na Seção \ref{sec:tiposDaltonismo}. Por esses motivos entende-se que as cores que a maioria dos daltônicos possuem dificuldade de enxergar estão entre o vermelho e o verde. Afim de deslocar os valores próximos a $60^o$, os valores do intervalo $[0^o;60^o]$ foram compactados no intervalo $[320^o;360^o$] e todos os valores do intervalo $[240^o;360^o]$ foram compactados no intervalo $[240^o;320^o[$. Analogamente, os valores do intervalo $]60^o;120^o]$ foram compactados no intervalo $[120^o; 160^o]$ e todos os valores do intervalo $[120^o;240^o[$ foram compactados no intervalo $]160^o;240^o[$. A Figura \ref{fig:reduzirHamarelo} ilustra esse processamento sobre a Figura \ref{fig:platesIshihara}, juntamente com os processamentos definidos nas Equações \ref{eq:reduzirS} e \ref{eq:reduzirV}. A Figura \ref{fig:figuraColorsHamarelo} ilustra os possíveis valores após distanciar os valores próximos a $60^o$. A Figura \ref{fig:reduzirHamareloHSI} ilustra esse processamento sobre a Figura \ref{fig:platesIshihara}, juntamente com os processamentos definidos nas Equações \ref{eq:reduzirS} e \ref{eq:reduzirI}.

\begin{figure}[!htb]
\centering \includegraphics[width=0.60\textwidth]{figuraColorsHamarelo.PNG}
\caption{Possíveis valores após distanciar valores próximos a $60^o$.}
\label{fig:figuraColorsHamarelo}
\end{figure}

\begin{figure}[!htb]
\centering \includegraphics[width=0.90\textwidth]{figuraDeslocarAzulHSV.jpg}
\caption{Deslocar valores de H próximos a $60^o$ no sistema HSV.}
\label{fig:reduzirHamarelo}
\end{figure}

\begin{figure}[!htb]
\centering \includegraphics[width=0.90\textwidth]{figuraDeslocarAzulHSI.jpg}
\caption{Deslocar valores de H próximos a $60^o$ no sistema HSI.}
\label{fig:reduzirHamareloHSI}
\end{figure}

Foi implementado um método que divide H em 360 intervalos e identifica qual intervalo possui mais incidências. Se um pixel é branco, ele não é contabilizado, pois o valor de H é indiferente. Nesse método, o intervalo $0$ representa $0^o$, o intervalo $1$ representa $1^o$ e assim sucessivamente. Após identificar esse valor, os valores são deslocados seguindo a mesma analogia do deslocamento dos valores de H próximos a $60^o$, no entanto os valores deslocados serão os que são próximos ao valor com maior número de incidencias encontrado. Por exemplo, na imagem original da Figura \ref{fig:figuraHistogramaHSV} o valor com $35^o$ é o com maior número de incidências, então os valores próximos a $35^o$ serão deslocados, que são os valores contidos no intervalo $[0^o;95^o] \cup [335^o;360^o]$. As Figuras \ref{fig:figuraHistogramaHSV}, \ref{fig:figuraHistogramaHSV2} e \ref{fig:figuraHistogramaHSV3} apresentam o resultado desse processamento em algumas pranchas de Ishihara. Esse método não apresenta bons resultados com a Figura \ref{fig:platesIshihara} porque ele deve encontrar a cor mais comum em cada uma das pranchas.


\begin{figure}[!htb]
\centering \includegraphics[width=0.9\textwidth]{figuraHistograma.png}
\caption{Resultado do deslocando dos valores próximos a $35^o$ e limitando a quantidade de valores para saturação e valor ou intensidade.}  \label{fig:figuraHistogramaHSV}
\end{figure}

\begin{figure}[!htb]
\centering \includegraphics[width=0.9\textwidth]{figuraHistograma2.png}
\caption{Resultado do deslocando dos valores próximos a $41^o$ e limitando a quantidade de valores para saturação e valor ou intensidade.} \label{fig:figuraHistogramaHSV2}
\end{figure}

\begin{figure}[!htb]
\centering \includegraphics[width=0.9\textwidth]{figuraHistograma3.png}
\caption{Resultado do deslocando dos valores próximos a $56^o$ e limitando a quantidade de valores para saturação e valor ou intensidade.} \label{fig:figuraHistogramaHSV3}
\end{figure}

Os processamentos apresentados nesta Subseção limitam as cores, tornando mais fácil a identificação das diferenças entre as cores pela maioria dos daltônicos. No entanto, devido a impactante abstração que foi realizada nos sistemas de cores HSV e HSI, muitas características são perdidas, o que faz com que os valores dentro de algumas pranchas não possam ser identificados. Em alguns casos, a identificação das cores torna-se mais complicada até mesmo para pessoas que não são portadoras de daltonismo.
...
\begin{figure}[!htb]
\centering \includegraphics[width=0.90\textwidth]{figuraElevarV.jpg}
\caption{Processamento da Equação \ref{eq:elevarV} sobre a Figura \ref{fig:platesIshihara}. \label{fig:figuraElevarV}}
\end{figure}

\begin{figure}[!htb]
\centering \includegraphics[width=0.90\textwidth]{figuraElevarI.jpg}
\caption{Processamento da Equação \ref{eq:elevarI} sobre a Figura \ref{fig:platesIshihara}. \label{fig:figuraElevarI}}
\end{figure}
...
\begin{figure}[!htb]
\centering \includegraphics[width=0.90\textwidth]{figuraElevarS.jpg}
\caption{Processamento da Equação \ref{eq:elevarS} sobre a Figura \ref{fig:platesIshihara}, com $z = 3$. \label{fig:figuraElevarS}}
\end{figure}

\begin{figure}[!htb]
\centering \includegraphics[width=0.90\textwidth]{figuraElevarSeV.jpg}
...
\begin{figure}[!htb]
\centering \includegraphics[width=0.90\textwidth]{figuraElevarSeI.jpg}
...
\begin{figure}[!htb]
\centering \includegraphics[width=0.9\textwidth]{platesIshihara.png}
